...
# ----------------------------
# Sequencer / solenoid settings
# ----------------------------
POSITION_TOL_DEG = 15.0      # 到位容差
POSITION_WAIT_S = 1.0        # 最多等待到位时间
SOLENOID_ON_S = 0.8        # solenoid 按下持续时间
...
class Sequencer(threading.Thread):
    ...
    def _wait_until_position(self, target_angle: float, tol_deg: float, timeout_s: float) -> bool:
        """Wait until actual position is close enough to target. Returns True if reached."""
    ...
    def run(self):
        ...
        for idx, (note, duration) in enumerate(self.song):
            ...
            # 2) wait until near target position
            ...
            if not reached:
                self.out_q.put(("error", f"[Sequencer] Position timeout on {note} ({angle:+.1f}°)"))
            ...
            # 4) hold solenoid
            sol_t0 = time.perf_counter()
            while not self.stop_evt.is_set() and (time.perf_counter() - sol_t0 < SOLENOID_ON_S):
                time.sleep(0.002)
            ...
            # 6) wait until this note duration ends
            while not self.stop_evt.is_set() and (time.perf_counter() - note_start < duration):
                time.sleep(0.005)
