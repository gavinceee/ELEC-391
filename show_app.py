...
class ShowApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        right = tk.Frame(content, bg="#11161e", width=320)
    ...
    def _seq_reset(self):
        ...
        try:
            if self.reader and self.reader.is_alive():
                self.reader.write("SL=0!")
                self.reader.write(RESET_HOME_CMD)
        except Exception:
            pass
        ...
        try:
            while True:
                ...
                if kind == "line":
                    ...
                    move_time = payload.get("move_time", 0.0)
                    reached = payload.get("reached", False)
                    self._last_move_time = move_time
                    self._last_move_reached = reached
